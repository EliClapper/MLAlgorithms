...
OLS <- function(y, x){
  ...
  return(c(weights,dfs))
}
OLS(Y, X)
```

**below is not correct, this does not gives estimates for multivariate regression, but for simple linear and then for every variable in X separately. The intercept makes no sense** 
```{r}
...
```
lets see how the function does with X on Y and 3 variables on Y:
```{r}
OLS(Y, X)
