...
One of those functions is the 'logit' functions. To obtain this, we firstly do not model Y itself, but $P(Y = 1|X)$, denoted as p(x). this makes Y continuous. This probability can be written by taking the sigmoid function of x:
$$
\begin{align}
...
lines(px(seq(min(X1), max(X1), length.out = length(X1)), b0, beta), col = 'red')
points(20, 0.4, col = "blue", pch = 19)

```
The dots are represent the probabilities of Y being 1 given the value on X for that dot. For example, the blue dot at (20, 0.4) will have a 40% probability of belonging to Y = 1, rather than Y = 0. The red line represents the line that would be fit if the interval between all values in X would be equal.
...
and even further to finally obtain logit function:
$$
\ln(\frac{p(x)}{1-p(x)}) = \beta_0 + \beta_1X
$$

And so we rewrote Y as a linear function of X by first modeling the probability of Y = 1 given X and than taking the log odds of this probability. This means that a change in X, does not increase p(x) with $\beta$, but rather the log odds of p(x). 
...
Maximizing the log likelihood is equal to minimizing the cost function:
$$
C(\boldsymbol{\beta}) = -\frac{\sum^n_{i=1}y_i*(\log(S_i) + (1-y_i))*\log(1-S_i)}{n}
$$
This tests how well the predictor space can predict the classes. $C(\boldsymbol{\beta})$ gets lower as X becomes a better predictor for X.
...
GradDescBern <- function(Y, X, rate, thresh, maxit){
  ...
  Y <- as.matrix(Y) #set Y as matrix for matrix algebra
  
  iter <- pred_rate <- 0 #intial prediction rate and iterion = 0
...
Now we make a traceplot of our estimates and add the `glm()` estimates as constants.
```{r}
